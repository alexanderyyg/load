...
#include "ImportAndSaveUI.generated.h"
//此文件虽然可以传递的变量几乎都被注释掉了，但可以作为框架为未来可能需要扩充的变量做准备
/** Import mesh type */
UENUM()
enum EFBXBuilderImportTypeB
{
	/** Select Static Mesh if you'd like to import static mesh. */
	FBXBuilderIT_StaticMeshB UMETA(DisplayName="Static Mesh"),
	/** Select Skeletal Mesh if you'd like to import skeletal mesh. */
	FBXBuilderIT_SkeletalMeshB UMETA(DisplayName="Skeletal Mesh"),
	/** Select Animation if you'd like to import only animation. */
	FBXBuilderIT_AnimationB UMETA(DisplayName="Animation"),
...
class UImportAndSaveUI : public UObject//, public IImportSettingsParser
{
public:	
	...
	UImportAndSaveUI(const FObjectInitializer& ObjectInitializer);//在load.cpp中定义
//	UImportAndSaveUI(const FObjectInitializer& ObjectInitializer);
//	UImportAndSaveUI(const FObjectInitializer& ObjectInitializer);
