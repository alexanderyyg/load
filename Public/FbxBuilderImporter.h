...
class UAnimSequence;
class UFbxBuilderSkeletalMeshImportData;
...
DECLARE_LOG_CATEGORY_EXTERN(LogFbxBuilder, Log, All);
...
struct FBXImportOptions
{
	...
	EFBXBuilderNormalImportMethod NormalImportMethod;
	EFBXBuilderNormalGenerationMethod::Type NormalGenerationMethod;
	bool bTransformVertexToAbsolute;
	bool bBakePivotInVertex;
	EFBXBuilderImportType ImportType;
	// Static Mesh options
	bool bCombineToSingle;
	EBuilderVertexColorImportOption::Type VertexColorImportOption;
...
//	EFBXBuilderAnimationLengthImportType AnimationLengthImportType;
	...
	*/
	...
	{
		return NormalImportMethod == FBXBuilderNIM_ImportNormals || NormalImportMethod == FBXBuilderNIM_ImportNormalsAndTangents;
	}

	bool ShouldImportTangents()
	{
		return NormalImportMethod == FBXBuilderNIM_ImportNormalsAndTangents;
	}
/*
	void ResetForReimportAnimation()
	{
		bImportMorph = true;
		AnimationLengthImportType = FBXBuilderALIT_ExportedTime;
	}
*/
...
public:
	...
	//Name API
	LOAD_API void GetAllNodeNameArray(TArray<FString> &AllNodeNames) const;//LOAD_API
	LOAD_API void GetAnimatedNodeNameArray(TArray<FString> &AnimatedNodeNames) const; //LOAD_API
	LOAD_API void GetNodeAnimatedPropertyNameArray(const FString &NodeName, TArray<FString> &AnimatedPropertyNames) const;//LOAD_API
	LOAD_API void GetCurveData(const FString& NodeName, const FString& PropertyName, int32 ChannelIndex, int32 CompositeIndex, FInterpCurveFloat& CurveData, bool bNegative) const;//LOAD_API
	LOAD_API void GetBakeCurveData(const FString& NodeName, const FString& PropertyName, int32 ChannelIndex, int32 CompositeIndex, TArray<float>& CurveData, float PeriodTime, float StartTime = 0.0f, float StopTime= -1.0f, bool bNegative = false) const;//LOAD_API

	//Handle API
	LOAD_API void GetAllNodePropertyCurveHandles(const FString& NodeName, const FString& PropertyName, TArray<FFbxAnimCurveHandle> &PropertyCurveHandles) const;//LOAD_API
	LOAD_API void GetCurveHandle(const FString& NodeName, const FString& PropertyName, int32 ChannelIndex, int32 CompositeIndex, FFbxAnimCurveHandle &CurveHandle) const;//LOAD_API
	LOAD_API void GetCurveData(const FFbxAnimCurveHandle &CurveHandle, FInterpCurveFloat& CurveData, bool bNegative) const;//LOAD_API
	LOAD_API void GetBakeCurveData(const FFbxAnimCurveHandle &CurveHandle, TArray<float>& CurveData, float PeriodTime, float StartTime = 0.0f, float StopTime = -1.0f, bool bNegative = false) const;//LOAD_API

	//Conversion API
	LOAD_API void GetConvertedTransformCurveData(const FString& NodeName, FInterpCurveFloat& TranslationX, FInterpCurveFloat& TranslationY, FInterpCurveFloat& TranslationZ,
													 ...
													 FTransform& DefaultTransform) const;//LOAD_API
...
FBXImportOptions* GetImportOptions( class FFbxBuilderImporter* FbxImporter, UFbxBuilderImportUI* ImportUI, bool bShowOptionDialog, bool bIsAutomated, const FString& FullPath, bool& OutOperationCanceled, bool& OutImportAll, bool bIsObjFormat, bool bForceImportType = false, EFBXBuilderImportType ImportType = FBXBuilderIT_StaticMesh, UObject* ReimportObject = nullptr);
void ApplyImportUIToImportOptions(UFbxBuilderImportUI* ImportUI, FBXImportOptions& InOutImportOptions);
...
public:
	 ...
	 */
	LOAD_API static FFbxBuilderImporter* GetInstance();//LOAD_API
	static void DeleteInstance();

	static FFbxBuilderImporter* GetPreviewInstance();
	static void DeletePreviewInstance();

	TArray<UAlexOBJ *> *AlexOBJS;
	/**
	 ...
	 */
	LOAD_API bool ImportFromFile(const FString& Filename, const FString& Type, bool bPreventMaterialNameClash = false);//LOAD_API
	...
	* @param LodIndex		The index of the LOD we search the mesh node
	*/
	LOAD_API void FindAllLODGroupNode(TArray<FbxNode*> &OutNodeInLod, FbxNode* NodeLodGroup, int32 LodIndex);//LOAD_API
	...
	*/
	 ...
	 */
	LOAD_API UStaticMesh* ImportStaticMesh(UObject* InParent, FbxNode* Node, const FName& Name, EObjectFlags Flags, UFbxBuilderStaticMeshImportData* ImportData, UStaticMesh* InStaticMesh = NULL, int LODIndex = 0, void *ExistMeshDataPtr = nullptr);
	...
	*/
	// LOAD_API
	 ...
	 LOAD_API UStaticMesh* ImportStaticMeshAsSingle(UObject* InParent, TArray<FbxNode*>& MeshNodeArray, const FName InName, EObjectFlags Flags, UFbxBuilderStaticMeshImportData* TemplateImportData, UStaticMesh* InStaticMesh, int LODIndex = 0, void *ExistMeshDataPtr = nullptr);//LOAD_API
	...
	*/
	 LOAD_API void PostImportStaticMesh(UStaticMesh* StaticMesh, TArray<FbxNode*>& MeshNodeArray);//LOAD_API
    //LOAD_API
	static void UpdateStaticMeshImportData(UStaticMesh *StaticMesh, UFbxBuilderStaticMeshImportData* StaticMeshImportData);
//	static void UpdateSkeletalMeshImportData(USkeletalMesh *SkeletalMesh, UFbxBuilderSkeletalMeshImportData* SkeletalMeshImportData, int32 SpecificLod, TArray<FName> *ImportMaterialOriginalNameData, TArray<FBuilderImportMeshLodSectionsData> *ImportMeshLodData);
	...
	UStaticMesh* ReimportStaticMesh(UStaticMesh* Mesh, UFbxBuilderStaticMeshImportData* TemplateImportData);
	...
	* @return UObject* the new Unreal mesh object
	*/
	UStaticMesh* ReimportSceneStaticMesh(uint64 FbxNodeUniqueId, uint64 FbxMeshUniqueId, UStaticMesh* Mesh, UFbxBuilderStaticMeshImportData* TemplateImportData);
	...
	* @return UObject* the new Unreal mesh object
	*/
//	USkeletalMesh* ReimportSkeletalMesh(USkeletalMesh* Mesh, UFbxBuilderSkeletalMeshImportData* TemplateImportData, uint64 SkeletalMeshFbxUID = 0xFFFFFFFFFFFFFFFF, TArray<FbxNode*> *OutSkeletalMeshArray = nullptr);
	...
	public:
		...
		FName Name;
		EObjectFlags Flags;
		UFbxBuilderSkeletalMeshImportData* TemplateImportData;
		int32 LodIndex;
		...
		FBuilderImportMeshLodSectionsData *ImportMeshSectionsData;
	};
	*/

//	LOAD_API USkeletalMesh* ImportSkeletalMesh(FImportSkeletalMeshArgs &ImportSkeletalMeshArgs);//LOAD_API
	 ...
	 */
//源码版有
//	UAnimSequence* ImportAnimations(USkeleton* Skeleton, UObject* Outer, TArray<FbxNode*>& SortedLinks, const FString& Name, UFbxBuilderAnimSequenceImportData* TemplateImportData, TArray<FbxNode*>& NodeArray);
...
//	void ImportFbxMorphTarget(TArray<FbxNode*> &SkelMeshNodeArray, USkeletalMesh* BaseSkelMesh, UObject* Parent, int32 LODIndex, const FSkeletalMeshImportData &BaseSkeletalMeshImportData);
	 ...
	 */
//	bool ImportSkeletalMeshLOD(USkeletalMesh* InSkeletalMesh, USkeletalMesh* BaseSkeletalMesh, int32 DesiredLOD, bool bNeedToReregister = true, TArray<UActorComponent*>* ReregisterAssociatedComponents = NULL, UFbxBuilderSkeletalMeshImportData* TemplateImportData = nullptr);
	 ...
	 */
	LOAD_API void ReleaseScene();//LOAD_API
	...
	*/
//	LOAD_API void FillFbxSkelMeshArrayInScene(FbxNode* Node, TArray< TArray<FbxNode*>* >& outSkelMeshArray, bool ExpandLOD, bool bForceFindRigid = false);//LOAD_API
...
//	FbxNode* FindFBXMeshesByBone(const FName& RootBoneName, bool bExpandLOD, TArray<FbxNode*>& OutFBXMeshNodeArray);
	...
	*/
	LOAD_API void FillFbxCollisionMeshArray(FbxNode* Node);//LOAD_API
	...
	*/
	LOAD_API void FillFbxMeshArray(FbxNode* Node, TArray<FbxNode*>& outMeshArray, UnFbxB::FFbxBuilderImporter* FFbxBuilderImporter);//LOAD_API
	...
	*/
	LOAD_API void FillFbxMeshAndLODGroupArray(FbxNode* Node, TArray<FbxNode*>& outLODGroupArray, TArray<FbxNode*>& outMeshArray);//LOAD_API
...
//	FbxNode* GetRootSkeleton(FbxNode* Link);
	 ...
	 */
	LOAD_API FBXImportOptions* GetImportOptions() const;//LOAD_API

	/*
	* This function show a dialog to let the user know what will be change if the fbx is imported
	*/
//源码的一个大分支	void ShowFbxReimportPreview(UObject *ReimportObj, UFbxBuilderImportUI* ImportUI, const FString& FullPath);

	/*
	* Function use to retrieve general fbx information for the preview
	*/
//源码的一个大分支	void FillGeneralFbxFileInformation(void *GeneralInfoPtr);

	/** helper function **/
	LOAD_API static void DumpFBXNode(FbxNode* Node);//LOAD_API
	 ...
	 */
	void ApplyTransformSettingsToFbxNode(FbxNode* Node, UFbxBuilderAssetImportData* AssetData);
	 ...
	 * @param AssetData the asset data object to get transform data from
	 */
	void RemoveTransformSettingsFromFbxNode(FbxNode* Node, UFbxBuilderAssetImportData* AssetData);
	 ...
	 */
	void BuildFbxMatrixForImportTransform(FbxAMatrix& OutMatrix, UFbxBuilderAssetImportData* AssetData);
...
//	void ImportMorphTargetsInternal( TArray<FbxNode*>& SkelMeshNodeArray, USkeletalMesh* BaseSkelMesh, UObject* Parent, int32 LODIndex, const FSkeletalMeshImportData &BaseSkeletalMeshImportData);
	...
	*/
//	void InsertNewLODToBaseSkeletalMesh(USkeletalMesh* InSkeletalMesh, USkeletalMesh* BaseSkeletalMesh, int32 DesiredLOD, UFbxBuilderSkeletalMeshImportData* TemplateImportData);
...
protected:
	...
	static TSharedPtr<FFbxBuilderImporter> StaticInstance;
	static TSharedPtr<FFbxBuilderImporter> StaticPreviewInstance;
...
public:
	...
	* @returns bool*	true if import successfully.
	*/
//	bool FillSkeletalMeshImportData(TArray<FbxNode*>& NodeArray, UFbxBuilderSkeletalMeshImportData* TemplateImportData, TArray<FbxShape*> *FbxShapeArray, FSkeletalMeshImportData* OutData, TArray<FName> &LastImportedMaterialNames);
...
//	bool GatherPointsForMorphTarget(FSkeletalMeshImportData* OutData, TArray<FbxNode*>& NodeArray, TArray< FbxShape* >* FbxShapeArray, TSet<uint32>& ModifiedPoints);
	 ...
	 */
//	bool ImportBone(TArray<FbxNode*>& NodeArray, FSkeletalMeshImportData &ImportData, UFbxBuilderSkeletalMeshImportData* TemplateData, TArray<FbxNode*> &OutSortedLinks, bool& bOutDiffPose, bool bDisableMissingBindPoseWarning, bool & bUseTime0AsRefPose, FbxNode *SkeletalMeshNode);
...
//	int32 DoUnSmoothVerts(FSkeletalMeshImportData &ImportData, bool bDuplicateUnSmoothWedges = true);
	...
	*/
	...
	//
public:
//	LOAD_API void PopulateAnimatedCurveData(FFbxCurvesAPI &CurvesAPI);//LOAD_API
...
public:
	 ...
	 */
//	LOAD_API bool HasUnknownCameras( AMatineeActor* InMatineeActor ) const;//LOAD_API
...
//	}
	 ...
	 */
//	LOAD_API bool ImportMatineeSequence(AMatineeActor* InMatineeActor);//LOAD_API
...
//	void SetupAnimationDataFromMesh(USkeletalMesh * SkeletalMesh, UObject* InParent, TArray<FbxNode*>& NodeArray, UFbxBuilderAnimSequenceImportData* ImportData, const FString& Filename);

	/** error message handler */
//	LOAD_API void AddTokenizedErrorMessage(TSharedRef<FTokenizedMessage> Error, FName FbxErrorName );//LOAD_API
...
/*
LOAD_API class  FFbxLogger////LOAD_API
{
	FFbxLogger();
	~FFbxLogger();
	...
	friend class FFbxBuilderImporter;
//	friend class FFbxLoggerSetter;
};
*/
/**
...
*/
/*
LOAD_API class  FFbxLoggerSetter//LOAD_API
{
	class FFbxLogger Logger;
	FFbxBuilderImporter * Importer;

public:
	FFbxLoggerSetter(FFbxBuilderImporter * InImpoter, bool ShowLogMessageOnlyIfError = false)
		: Importer(InImpoter)
	{
