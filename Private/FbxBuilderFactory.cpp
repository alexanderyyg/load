...
#define LOCTEXT_NAMESPACE "FBXBuilderFactory"

UFbxBuilderFactory::UFbxBuilderFactory(const FObjectInitializer& ObjectInitializer)
...
UObject* UFbxBuilderFactory::ImportObject(UClass* InClass, UObject* InOuter, FName InName, EObjectFlags InFlags, const FString& Filename, const TCHAR* Parms, bool& OutCanceled)
{
	if (AlexOBJS)
		return UFactory::ImportObject(InClass, InOuter, InName, InFlags, Filename, Parms, OutCanceled);
	else
	{
		UFbxFactory *StaticFbxFactory;
		...
		UObject* ImportedModel = StaticFbxFactory->ImportObject(InClass, InOuter, InName, InFlags, Filename, Parms, OutCanceled);
...
void UFbxBuilderFactory::PostInitProperties()
{
	Super::PostInitProperties();
	bEditorImport = true;
	bText = false;

	ImportUI = NewObject<UFbxBuilderImportUI>(this, NAME_None, RF_NoFlags);
}


bool UFbxBuilderFactory::DoesSupportClass(UClass * Class)
{
...
UClass* UFbxBuilderFactory::ResolveSupportedClass()
{
//	UClass* ImportClass = NULL;

	UClass* ImportClass = UStaticMesh::StaticClass();
	/*
	if (ImportUI->MeshTypeToImport == FBXBuilderIT_SkeletalMesh)
	{
		ImportClass = USkeletalMesh::StaticClass();
	}
	else if (ImportUI->MeshTypeToImport == FBXBuilderIT_Animation)
	{
...
bool UFbxBuilderFactory::DetectImportType(const FString& InFilename)
{
	UnFbxB::FFbxBuilderImporter* FFbxImporter = UnFbxB::FFbxBuilderImporter::GetInstance();
	FFbxImporter->AlexOBJS = AlexOBJS;
//	UnFbxB::FFbxLoggerSetter Logger(FFbxImporter);
...
//	{
//		ImportUI->MeshTypeToImport = EFBXBuilderImportType(ImportType);
...
UObject* UFbxBuilderFactory::ImportANode(void* VoidFbxImporter, TArray<void*> VoidNodes, UObject* InParent, FName InName, EObjectFlags Flags, int32& NodeIndex, int32 Total, UObject* InMesh, int LODIndex)
{
	UnFbxB::FFbxBuilderImporter* FFbxImporter = (UnFbxB::FFbxBuilderImporter*)VoidFbxImporter;
...
bool UFbxBuilderFactory::ConfigureProperties()
{
	bDetectImportTypeOnImport = true;
	EnableShowOption();
...
UObject* UFbxBuilderFactory::FactoryCreateBinary
(
 UClass*			Class,
 UObject*			InParent,
 FName				Name,
 ...
 )
{
	...
	UFbxBuilderStaticMeshImportData* ExistingStaticMeshImportData = nullptr;
...
*/
	...
	UnFbxB::FFbxBuilderImporter* FbxImporter = UnFbxB::FFbxBuilderImporter::GetInstance();
	FbxImporter->AlexOBJS = AlexOBJS;
...
//	UnFbxB::FFbxLoggerSetter Logger(FbxImporter);

	EFBXBuilderImportType ForcedImportType = FBXBuilderIT_StaticMesh;
	...
	canchangewithmini=ImportUI->bCanChangeWithMiniNum;
...
//			ImportUI->MeshTypeToImport = FBXBuilderIT_StaticMesh;
/*
			if ( ImportUI->MeshTypeToImport == FBXBuilderIT_SkeletalMesh )
			{
		...
		//	else if( ImportUI->MeshTypeToImport == FBXBuilderIT_StaticMesh )
			...
			if (true)// ImportUI->MeshTypeToImport == FBXBuilderIT_StaticMesh)
			{
				...
				//观察是Maya冻结坐标以后的状态还是Max导出时候LocalRotationPivot=0的样子
				/*
				FbxImporter->IsMaya = false;
				if (strcmp(RootNodeToImport->GetName(),"RootNode")==0)
				{
					
					for (int32 childcount = 0; childcount < RootNodeToImport->GetChildCount(true); childcount++)
					{
						FbxNode * newNode = RootNodeToImport->GetChild(childcount);
						if (newNode->GetMesh())
						{
							FbxDouble3 CurrentPivot = newNode->RotationPivot.Get();
							if ((CurrentPivot[0] != 0) || (CurrentPivot[1] != 0) || (CurrentPivot[1] != 0))
							{
								FbxImporter->IsMaya = true;
								break;
							}
							//	if(NewNode)
						}
			...
			{  
				...
				if (true)// ImportUI->MeshTypeToImport == FBXBuilderIT_StaticMesh )  // static mesh
				{
...
*/	
					...
					else
					{
						...
						for ( auto AssetIt = AllNewAssets.CreateConstIterator(); AssetIt; ++AssetIt )
						{
							...
							UStaticMesh* AssetMesh = Cast<UStaticMesh>(Asset);

							if ( Asset != NewStaticMesh )
							{
								FAssetRegistryModule::AssetCreated(Asset);
						//		FAssetRegistryModule::AssetCreated(AssetMesh);
								Asset->MarkPackageDirty();
						//		AssetMesh->GetOutermost()->MarkPackageDirty();
							}
						}
			...
			/*
				else if ( ImportUI->MeshTypeToImport == FBXBuilderIT_SkeletalMesh )// skeletal mesh
				{
					...
					for (int32 i = 0; i < SkelMeshArray.Num(); i++)
					{
						...
						for (LODIndex = 0; LODIndex < MaxLODLevel; LODIndex++)
						{
							...
							if (LODIndex == 0 && SkelMeshNodeArray.Num() != 0)
							{
								...
								UnFbxB::FFbxBuilderImporter::FImportSkeletalMeshArgs ImportSkeletalMeshArgs;
						...
						if (NewObject)
						{
							...
							UnFbxB::FFbxBuilderImporter::UpdateSkeletalMeshImportData(SkeletalMesh, ImportUI->SkeletalMeshImportData, INDEX_NONE, nullptr, nullptr);
				...
				else if ( ImportUI->MeshTypeToImport == FBXBuilderIT_Animation )// animation
				{
...
UObject* UFbxBuilderFactory::RecursiveImportNode(void* VoidFbxImporter, void* VoidNode, UObject* InParent, FName InName, EObjectFlags Flags, int32& NodeIndex, int32 Total, TArray<UObject*>& OutNewAssets)
{
	TArray<void*> TmpVoidArray;
	UObject* NewObject = NULL;
	UnFbxB::FFbxBuilderImporter *FbxImporter = (UnFbxB::FFbxBuilderImporter *)VoidFbxImporter;
...
void UFbxBuilderFactory::CleanUp() 
{
	UnFbxB::FFbxBuilderImporter* FFbxImporter = UnFbxB::FFbxBuilderImporter::GetInstance();
	bDetectImportTypeOnImport = true;
	bShowOption = true;
	...
	if (FFbxImporter)
	{
		struct UnFbxB::FBXImportOptions* ImportOptions = FFbxImporter->GetImportOptions();
	/*
		if ( ImportOptions )
		{
...
bool UFbxBuilderFactory::FactoryCanImport(const FString& Filename)
{
...
/*
IImportSettingsParser* UFbxBuilderFactory::GetImportSettingsParser()
{
	return ImportUI;
}
*/
