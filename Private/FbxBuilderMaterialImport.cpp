...
DEFINE_LOG_CATEGORY_STATIC(LogFbxBuilderMaterialImport, Log, All);

#define LOCTEXT_NAMESPACE "FbxBuilderMaterialImport"

using namespace UnFbxB;
/*
UTexture* UnFbxB::FFbxBuilderImporter::ImportTexture(FbxFileTexture* FbxTexture, bool bSetupAsNormalMap)
{
	if (!FbxTexture)
	{
		return nullptr;
	...
	{
		UE_LOG(LogFbxBuilderMaterialImport, Warning, TEXT("Unable to find Texture file %s"), *AbsoluteFilename);
	}
	...
	if (DataBinary.Num()>0)
	{
		UE_LOG(LogFbxBuilderMaterialImport, Verbose, TEXT("Loading texture file %s"),*FinalFilePath);
		...
		if ( bSetupAsNormalMap )
		{
			...
			{
				UE_LOG(LogFbxBuilderMaterialImport, Warning, TEXT("Manual texture reimport and recompression may be needed for %s"), *TextureName);
			}
		}
...
*/
/*
void UnFbxB::FFbxBuilderImporter::ImportTexturesFromNode(FbxNode* Node)
{
...
void LogPropertyAndChild(FbxSurfaceMaterial& FbxMaterial, const FbxProperty &Property)
{
	...
	UE_LOG(LogFbxBuilderMaterialImport, Display, TEXT("Property Name [%s]"), UTF8_TO_TCHAR(PropertyName.Buffer()));
	...
	if (TextureCount > 0)
	{
		for (int32 TextureIndex = 0; TextureIndex < TextureCount; ++TextureIndex)
		{
			...
			{
				UE_LOG(LogFbxBuilderMaterialImport, Display, TEXT("Texture Path [%s]"), UTF8_TO_TCHAR(TextureObj->GetFileName()));
			}
		}
	}
...
bool UnFbxB::FFbxBuilderImporter::CreateAndLinkExpressionForMaterialProperty(
							...
							const FVector2D& Location)
{
	...
	if( FbxProperty.IsValid() )
	{
		...
		{
			UE_LOG(LogFbxBuilderMaterialImport, Warning,TEXT("Layered or procedural Textures are not supported (material %s)"),UTF8_TO_TCHAR(FbxMaterial.GetName()));
		}
		else
		{
...
//
//-------------------------------------------------------------------------
void UnFbxB::FFbxBuilderImporter::FixupMaterial( FbxSurfaceMaterial& FbxMaterial, UMaterial* UnrealMaterial )
{
...
FString UnFbxB::FFbxBuilderImporter::GetMaterialFullName(FbxSurfaceMaterial& FbxMaterial)
{
...
bool UnFbxB::FFbxBuilderImporter::LinkMaterialProperty(
	FbxSurfaceMaterial& FbxMaterial,
	...
	if (FbxProperty.IsValid())
	{
		...
		if (LayeredTextureCount > 0)
		{
			UE_LOG(LogFbxBuilderMaterialImport, Warning, TEXT("Layered Textures are not supported (material %s)"), UTF8_TO_TCHAR(FbxMaterial.GetName()));
		}
		else
		{
			int32 TextureCount = FbxProperty.GetSrcObjectCount<FbxTexture>();
...
void UnFbxB::FFbxBuilderImporter::CreateUnrealMaterial(FbxSurfaceMaterial& FbxMaterial, TArray<UMaterialInterface*>& OutMaterials, TArray<FString>& UVSets, bool bForSkeletalMesh)
{
...
//				FFbxErrors::Generic_LoadingSceneFailed);
	...
	else
	{
		...
		if (UnrealMaterial != NULL)
		{
			...
			{
				UE_LOG(LogFbxBuilderMaterialImport, Display, TEXT("Creating Material [%s]"), UTF8_TO_TCHAR(FbxMaterial.GetName()));
				LogPropertyAndChild(FbxMaterial, FirstProperty);
				UE_LOG(LogFbxBuilderMaterialImport, Display, TEXT("-------------------------------"));
			}
#endif
...
int32 UnFbxB::FFbxBuilderImporter::CreateNodeMaterials(FbxNode* FbxNode, TArray<UMaterialInterface*>& OutMaterials, TArray<FString>& UVSets, bool bForSkeletalMesh)
{
