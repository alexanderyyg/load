...
/*=============================================================================
	Main implementation of FFbxBuilderImporter : import FBX data to Unreal
=============================================================================*/
...
//#include "FbxBuilderSkeletalMeshImportData.h"
...
DEFINE_LOG_CATEGORY(LogFbxBuilder);

#define LOCTEXT_NAMESPACE "FbxBuilderMainImport"

namespace UnFbxB
{

TSharedPtr<FFbxBuilderImporter> FFbxBuilderImporter::StaticInstance;

TSharedPtr<FFbxBuilderImporter> FFbxBuilderImporter::StaticPreviewInstance;



FBXImportOptions* GetImportOptions( UnFbxB::FFbxBuilderImporter* FbxImporter, UFbxBuilderImportUI* ImportUI, bool bShowOptionDialog, bool bIsAutomated, const FString& FullPath, bool& OutOperationCanceled, bool& bOutImportAll, bool bIsObjFormat, bool bForceImportType, EFBXBuilderImportType ImportType, UObject* ReimportObject)
{
	...
	{
		bOutImportAll = false;
		UnFbxB::FBXImportOptions* ImportOptions = FbxImporter->GetImportOptions();
...
//		ImportUI->bImportAsSkeletal = ImportUI->MeshTypeToImport == FBXBuilderIT_SkeletalMesh;
	//	ImportUI->bImportMesh =  ImportUI->MeshTypeToImport;// != FBXBuilderIT_Animation;
...
//		ImportUI->AnimSequenceImportData->bImportMeshesInBoneHierarchy = ImportUI->SkeletalMeshImportData->bImportMeshesInBoneHierarchy;
	...
	/*
		auto OnPreviewFbxImportLambda = ImportUI->MeshTypeToImport == FBXBuilderIT_Animation ? nullptr : FOnPreviewFbxImport::CreateLambda([=]
		{
			UnFbxB::FFbxBuilderImporter* PreviewFbxImporter = UnFbxB::FFbxBuilderImporter::GetPreviewInstance();
			PreviewFbxImporter->ShowFbxReimportPreview(ReimportObject, ImportUI, FullPath);
			UnFbxB::FFbxBuilderImporter::DeletePreviewInstance();
		});
*/
		TSharedPtr<SFbxBuilderOptionWindow> FbxOptionWindow;
		Window->SetContent
		(
			SAssignNew(FbxOptionWindow, SFbxBuilderOptionWindow)
			.ImportUI(ImportUI)
			.WidgetWindow(Window)
			.FullPath(FText::FromString(FullPath))
			.ForcedImportType( bForceImportType ? TOptional<EFBXBuilderImportType>( ImportType ) : TOptional<EFBXBuilderImportType>() )
...
/*
		if (ImportUI->MeshTypeToImport == FBXBuilderIT_SkeletalMesh || ImportUI->MeshTypeToImport == FBXBuilderIT_Animation)
		{
...
//	InOutImportOptions.bImportMaterials = ImportUI->bImportMaterials;
	InOutImportOptions.bImportMaterials = 0;
...
//	InOutImportOptions.bImportTextures = ImportUI->bImportTextures;
	InOutImportOptions.bImportTextures = 0;
...
//	InOutImportOptions.ImportType = ImportUI->MeshTypeToImport;
	...
	if (true)// ImportUI->MeshTypeToImport == FBXBuilderIT_StaticMesh )
	{
		UFbxBuilderStaticMeshImportData* StaticMeshData	= ImportUI->StaticMeshImportData;
...
/*
	else if ( ImportUI->MeshTypeToImport == FBXBuilderIT_SkeletalMesh )
	{
		UFbxBuilderSkeletalMeshImportData* SkeletalMeshData	= ImportUI->SkeletalMeshImportData;
		...
		if(ImportUI->bImportAnimations)
		{
			...
			UFbxBuilderAnimSequenceImportData* AnimData	= ImportUI->AnimSequenceImportData;
	...
	{
		UFbxBuilderAnimSequenceImportData* AnimData	= ImportUI->AnimSequenceImportData;
		InOutImportOptions.NormalImportMethod = FBXBuilderNIM_ComputeNormals;
	...
	InOutImportOptions.bImportRigidMesh = ImportUI->OriginalImportType == FBXBuilderIT_StaticMesh && ImportUI->MeshTypeToImport == FBXBuilderIT_SkeletalMesh;
...
FFbxBuilderImporter::FFbxBuilderImporter()
	: Scene(NULL)
	, ImportOptions(NULL)
...
//
//-------------------------------------------------------------------------
FFbxBuilderImporter* FFbxBuilderImporter::GetInstance()
{
	if (!StaticInstance.IsValid())
	{
		StaticInstance = MakeShareable( new FFbxBuilderImporter() );
	}
	return StaticInstance.Get();
}

void FFbxBuilderImporter::DeleteInstance()
{
	StaticInstance.Reset();
}

FFbxBuilderImporter* FFbxBuilderImporter::GetPreviewInstance()
{
	if (!StaticPreviewInstance.IsValid())
	{
		StaticPreviewInstance = MakeShareable(new FFbxBuilderImporter());
	}
	return StaticPreviewInstance.Get();
}

void FFbxBuilderImporter::DeletePreviewInstance()
{
...
//
//-------------------------------------------------------------------------
void FFbxBuilderImporter::CleanUp()
{
...
//
//-------------------------------------------------------------------------
void FFbxBuilderImporter::ReleaseScene()
{
	if (Importer)
	{
		Importer->Destroy();
...
FBXImportOptions* UnFbxB::FFbxBuilderImporter::GetImportOptions() const
{
	return ImportOptions;
}

int32 FFbxBuilderImporter::GetImportType(const FString& InFilename)
{
	...
	if (OpenFile(Filename, true))
	{
		...
		for ( ItemIndex = 0; ItemIndex < Statistics.GetNbItems(); ItemIndex++ )
		{
			...
			UE_LOG(LogFbxBuilder, Log, TEXT("ItemName: %s, ItemCount : %d"), *NameBuffer, ItemCount);
		}
...
bool FFbxBuilderImporter::GetSceneInfo(FString Filename, FbxSceneInfo& SceneInfo, bool bPreventMaterialNameClash /*= false*/)
{
	bool Result = true;
...
void FFbxBuilderImporter::TraverseHierarchyNodeRecursively(FbxSceneInfo& SceneInfo, FbxNode *ParentNode, FbxNodeInfo &ParentInfo)
{
...
bool FFbxBuilderImporter::OpenFile(FString Filename, bool bParseStatistics, bool bForSceneInfo )
{
	bool Result = true;
	...
	{
		UE_LOG(LogFbxBuilder, Error,TEXT("Call to FbxImporter::Initialize() failed."));
		UE_LOG(LogFbxBuilder, Warning, TEXT("Error returned: %s"), UTF8_TO_TCHAR(Importer->GetStatus().GetErrorString()));

		if (Importer->GetStatus().GetCode() == FbxStatus::eInvalidFileVersion )
		{
			UE_LOG(LogFbxBuilder, Warning, TEXT("FBX version number for this FBX SDK is %d.%d.%d"),
...
void FFbxBuilderImporter::FixMaterialClashName()
{
...
bool FFbxBuilderImporter::ImportFile(FString Filename, bool bPreventMaterialNameClash /*=false*/)
{
	bool Result = true;
	...
	UE_LOG(LogFbxBuilder, Log, TEXT("Loading FBX Scene from %s"), *Filename);
	...
	if(bStatus)
	{
		UE_LOG(LogFbxBuilder, Log, TEXT("FBX Scene Loaded Succesfully"));
		CurPhase = IMPORTED;
	}
	else
	{
...
void FFbxBuilderImporter::ConvertScene()
{
...
//
//-------------------------------------------------------------------------
bool FFbxBuilderImporter::ImportFromFile(const FString& Filename, const FString& Type, bool bPreventMaterialNameClash /*= false*/)
{
	bool Result = true;
	...
	case IMPORTED:
		{
			...
			if( !Type.Equals( Obj, ESearchCase::IgnoreCase ) )
			{
				...
				if (DocInfo)
				{
					...
					{
						const static UEnum* FBXImportTypeEnum = FindObject<UEnum>(ANY_PACKAGE, TEXT("EFBXBuilderImportType"));
...
ANSICHAR* FFbxBuilderImporter::MakeName(const ANSICHAR* Name)
{
...
FString FFbxBuilderImporter::MakeString(const ANSICHAR* Name)
{
	return FString(ANSI_TO_TCHAR(Name));
}

FName FFbxBuilderImporter::MakeNameForMesh(FString InName, FbxObject* FbxObject)
{
	FName OutputName;
	...
	else
	{
		...
		do 
		{
			...
			else
			{
				...
				//新需求：修改_变-
		//		ComposeName = FString::Printf(TEXT("%s-%s"), *InName, UTF8_TO_TCHAR(NewName));
			}
			if (NameCount > 0)
			{
				ComposeName += TEXT("_") + FString::FromInt(NameCount);
				//新需求：修改_变-
		//		ComposeName += TEXT("-") + FString::FromInt(NameCount);
			}
			NameCount++;
...
/*
FbxAMatrix FFbxBuilderImporter::ComputeSkeletalMeshTotalMatrix(FbxNode* Node, FbxNode *RootSkeletalNode)
{
...
*/
FbxAMatrix FFbxBuilderImporter::ComputeTotalMatrix(FbxNode* Node)
{
	FbxAMatrix Geometry;
...
bool FFbxBuilderImporter::IsOddNegativeScale(FbxAMatrix& TotalMatrix)
{
...
*/
int32 FFbxBuilderImporter::GetFbxMeshCount( FbxNode* Node, bool bCountLODs, int32& OutNumLODGroups )
{
...
*/
void FFbxBuilderImporter::FillFbxCollisionMeshArray(FbxNode* Node)
{
	if (Node->GetMesh())
	{
...
*/
void FFbxBuilderImporter::FillFbxMeshArray(FbxNode* Node, TArray<FbxNode*>& outMeshArray, UnFbxB::FFbxBuilderImporter* FFbxBuilderImporter)
{
	if (Node->GetMesh())
	{
		if (!FFbxBuilderImporter->FillCollisionModelList(Node) && Node->GetMesh()->GetPolygonVertexCount() > 0)
		{ 
	...
	for (ChildIndex=0; ChildIndex<Node->GetChildCount(); ++ChildIndex)
	{
		FillFbxMeshArray(Node->GetChild(ChildIndex), outMeshArray, FFbxBuilderImporter);
	}
}

void FFbxBuilderImporter::FillFbxMeshAndLODGroupArray(FbxNode* Node, TArray<FbxNode*>& outLODGroupArray, TArray<FbxNode*>& outMeshArray)
{
	// Is this node an LOD group
	bool bLODGroup = Node->GetNodeAttribute() && Node->GetNodeAttribute()->GetAttributeType() == FbxNodeAttribute::eLODGroup;
...
void FFbxBuilderImporter::ValidateAllMeshesAreReferenceByNodeAttribute()
{
...
FbxNode *FFbxBuilderImporter::RecursiveGetFirstMeshNode(FbxNode* Node, FbxNode* NodeToFind)
{
...
void FFbxBuilderImporter::RecursiveGetAllMeshNode(TArray<FbxNode *> &OutAllNode, FbxNode* Node)
{
	if (Node->GetMesh() != nullptr)
	{
		OutAllNode.Add(Node);
...
FbxNode* FFbxBuilderImporter::FindLODGroupNode(FbxNode* NodeLodGroup, int32 LodIndex, FbxNode *NodeToFind)
{
...
void FFbxBuilderImporter::FindAllLODGroupNode(TArray<FbxNode*> &OutNodeInLod, FbxNode* NodeLodGroup, int32 LodIndex)
{
	check(NodeLodGroup->GetChildCount() >= LodIndex);
	FbxNode *ChildNode = NodeLodGroup->GetChild(LodIndex);
...
FbxNode *FFbxBuilderImporter::RecursiveFindParentLodGroup(FbxNode *ParentNode)
{
...
/*
void FFbxBuilderImporter::RecursiveFixSkeleton(FbxNode* Node, TArray<FbxNode*> &SkelMeshes, bool bImportNestedMeshes )
{
...
*/
/*
FbxNode* FFbxBuilderImporter::GetRootSkeleton(FbxNode* Link)
{
...
void FFbxBuilderImporter::DumpFBXNode(FbxNode* Node) 
{
	...
	if(Mesh)
	{
		UE_LOG(LogFbxBuilder, Log, TEXT("================================================="));
		UE_LOG(LogFbxBuilder, Log, TEXT("Dumping Node START [%s] "), *NodeName);
		int DeformerCount = Mesh->GetDeformerCount();
		UE_LOG(LogFbxBuilder, Log,TEXT("\tTotal Deformer Count %d."), *NodeName, DeformerCount);
		for(int i=0; i<DeformerCount; i++)
		{
			...
			UE_LOG(LogFbxBuilder, Log,TEXT("\t\t[Node %d] %s (Type %s)."), i+1, *DeformerName, *DeformerTypeName);
			UE_LOG(LogFbxBuilder, Log,TEXT("================================================="));
		}
		...
		if(NodeAttribute)
		{
			...
			UE_LOG(LogFbxBuilder, Log,TEXT("\tAttribute (%s) Type (%d)."), *NodeAttributeName, (int32)Type);
			...
			for (int i=0; i<NodeAttribute->GetNodeCount(); ++i)
			{
				...
				if (Child)
				{
					...
					UE_LOG(LogFbxBuilder, Log,TEXT("\t\t[Node Attribute Child %d] %s (Type %s)."), i+1, *ChildName, *ChildTypeName);
				}
			}

		}

		UE_LOG(LogFbxBuilder, Log,TEXT("Dumping Node END [%s]"), *NodeName);
	}
...
void FFbxBuilderImporter::ApplyTransformSettingsToFbxNode(FbxNode* Node, UFbxBuilderAssetImportData* AssetData)
{
	check(Node);
	check(AssetData);
...
void FFbxBuilderImporter::RemoveTransformSettingsFromFbxNode(FbxNode* Node, UFbxBuilderAssetImportData* AssetData)
{
	check(Node);
	check(AssetData);
...
void FFbxBuilderImporter::BuildFbxMatrixForImportTransform(FbxAMatrix& OutMatrix, UFbxBuilderAssetImportData* AssetData)
{
	if(!AssetData)
	{
...
*/
/*
void FFbxBuilderImporter::RecursiveFindFbxSkelMesh(FbxNode* Node, TArray< TArray<FbxNode*>* >& outSkelMeshArray, TArray<FbxNode*>& SkeletonArray, bool ExpandLOD)
{
...
*/
/*
void FFbxBuilderImporter::RecursiveFindRigidMesh(FbxNode* Node, TArray< TArray<FbxNode*>* >& outSkelMeshArray, TArray<FbxNode*>& SkeletonArray, bool ExpandLOD)
{
...
*/
/*
void FFbxBuilderImporter::FillFbxSkelMeshArrayInScene(FbxNode* Node, TArray< TArray<FbxNode*>* >& outSkelMeshArray, bool ExpandLOD, bool bForceFindRigid )
{
...
*/
/*
FbxNode* FFbxBuilderImporter::FindFBXMeshesByBone(const FName& RootBoneName, bool bExpandLOD, TArray<FbxNode*>& OutFBXMeshNodeArray)
{
...
void FFbxBuilderImporter::CheckSmoothingInfo(FbxMesh* FbxMesh)
{
...
//
//-------------------------------------------------------------------------
FbxNode* FFbxBuilderImporter::RetrieveObjectFromName(const TCHAR* ObjectName, FbxNode* Root)
{
	if (Scene)
	{
		if (!Root)
...
UFbxBuilderImportUI::UFbxBuilderImportUI(const FObjectInitializer& ObjectInitializer)
...
bool UFbxBuilderImportUI::CanEditChange(const UProperty* InProperty) const
{
