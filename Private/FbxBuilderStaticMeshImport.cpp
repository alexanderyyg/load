...
//		ImportTexturesFromNode(Node);
	...
	if (false && !bImportedCollision && StaticMesh)	//如果没有碰撞，默认给一个碰撞if didn't import collision automatically generate one
	{
...
//	int32 MeshIndex;
	...
	//将模型位置旋转归零，并记录归零前的位置旋转偏移
		...
		if (StrPar == "RootNode")
		{
			...
			NewNode->LclRotation.Set(Zero);//倒下才容易找到角度
